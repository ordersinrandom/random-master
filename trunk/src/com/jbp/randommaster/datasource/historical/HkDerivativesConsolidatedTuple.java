...
/**
 * The data tuple that takes an list of HkDerivativesTRTuple and consolidate them into useful data
 * such as traded volume, last/max/min/avg traded prices etc.
 * 
 *
 */
public class HkDerivativesConsolidatedTuple implements VanillaDerivativesDataTuple {
	 ...
	 */
	protected HkDerivativesConsolidatedTuple() {
		expiryMonth=null;
		underlying=null;
		strikePrice = 0.0;
	 ...
	 */
	public HkDerivativesConsolidatedTuple(YearMonth expiryMonth, String underlying, double strikePrice, 
											FuturesOptions futOpt, CallPut cp,
											double lastTradedPrice, double maxTradedPrice, double minTradedPrice, 
											double averagedPrice, double tradedVolume, int transactionsCount) {
	 ...
	 * @param original The list of HkDerivativesTRTuple that we are going to be consolidated. Expect having the same basic attributes such as expiry, strike etc.
	 * @return A HkDerivativesConsolidatedTuple or null if the input original is empty.
	 */
	public static HkDerivativesConsolidatedTuple consolidate(Iterable<HkDerivativesTRTuple> original) {
		
		HkDerivativesConsolidatedTuple result=new HkDerivativesConsolidatedTuple();
		
		result.tradedVolume = 0.0;
		result.averagedPrice = 0.0;
		result.transactionsCount = 0;
		for (HkDerivativesTRTuple t : original) {
			// expiry
			if (result.expiryMonth==null)
				result.expiryMonth = t.getExpiryMonth();
			else if (!result.expiryMonth.equals(t.getExpiryMonth()))
				throw new IllegalArgumentException("input data doesn't have the same expiry month: current="+result.expiryMonth+", input="+t.getExpiryMonth());
			// underlying
			if (result.underlying==null)
				result.underlying = t.getUnderlying();
			else if (!result.underlying.equals(t.getUnderlying()))
				throw new IllegalArgumentException("input data doesn't have the same underlying: current="+result.underlying+", input="+t.getUnderlying());
			// strike
			if (result.strikePrice==0.0)
				result.strikePrice = t.getStrikePrice();
			else if (result.strikePrice!=t.getStrikePrice())
				throw new IllegalArgumentException("input data doesn't have the same strike: current="+result.strikePrice+", input="+t.getStrikePrice());
			// futures or options
			if (result.futuresOrOptions==null)
				...
				throw new IllegalArgumentException("input data doesn't have the same futuresOrOptions: current="+result.futuresOrOptions+", input="+t.getFuturesOrOptions());
			
			// call or put
			if (result.callPut==null)
				result.callPut = t.getCallPut();
			else if (result.callPut!=t.getCallPut())
				throw new IllegalArgumentException("input data doesn't have the same callPut: current="+result.callPut+", input="+t.getCallPut());
	...
	/**
	 * Consolidated field
	 * @return The number of transaction we have observed in the given tuple collections.
	 */
	...
	public String toString() {
		...
		buf.append("HkDerivativesConsolidatedTuple { expiryMonth=");
	...
	public boolean equals(Object obj) {
		if (obj==null)
			return false;
		else if (obj instanceof HkDerivativesConsolidatedTuple) {
			HkDerivativesConsolidatedTuple t = (HkDerivativesConsolidatedTuple) obj;
			return expiryMonth.equals(t.expiryMonth)
					&& underlying.equals(t.underlying)
					&& strikePrice==t.strikePrice
					&& futuresOrOptions==t.futuresOrOptions
					&& callPut==t.callPut
					&& lastTradedPrice==t.lastTradedPrice
					&& maxTradedPrice==t.maxTradedPrice
					&& minTradedPrice==t.minTradedPrice
					&& averagedPrice==t.averagedPrice
	...
	@Override
	public int hashCode() {
		return expiryMonth.hashCode()
				^underlying.hashCode()
				^Double.valueOf(strikePrice).hashCode()
				^futuresOrOptions.hashCode()
				^callPut.hashCode()
				^Double.valueOf(lastTradedPrice).hashCode()
				^Double.valueOf(maxTradedPrice).hashCode()
				^Double.valueOf(minTradedPrice).hashCode()
				^Double.valueOf(averagedPrice).hashCode()
				^Integer.valueOf(transactionsCount).hashCode()
